...
# 화면 타이틀 설정
pygame.display.set_caption("허윤이의 슈퍼팡") # 게임 이름
...
# 배경 만들기
background = pygame.image.load(os.path.join(image_path, "background1.png")) # image_path\background.png
...
# 공 만들기 (4개 크기에 대해 따로 처리, 4개의 이미지를 리스트로 가지고 있게 처리)
ball_images = [
    pygame.image.load(os.path.join(image_path, "balloon1_1.png")),
    pygame.image.load(os.path.join(image_path, "balloon2_1.png")),
    pygame.image.load(os.path.join(image_path, "balloon3_1.png")),
    pygame.image.load(os.path.join(image_path, "balloon4_1.png"))]

# 공 크기에 따른 최초 스피드
ball_speed_y = [-18, -15, -12, -9] # ball_images 의 index 0, 1, 2, 3 에 해당하는 최초 y 축 스피드
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ball_pos_x = ball_val["pos_x"] # balls list 에 있는 dic 에서 "pos_x" 키의 값을 가져옴 
