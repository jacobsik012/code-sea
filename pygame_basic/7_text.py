...
while running:
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하면 for 문의 event 로 가져옴
    ...
    # 충돌 처리를 위한 rect 정보 업데이트 (실제로 움직인 좌표로 렉탱글 좌표를 업데이트 해줘야 함)
    character_rect = character.get_rect() # 캐릭터가 갖는 렉탱글 정보를 가져 옴
    character_rect.left = character_x_pos
    character_rect.top = character_y_pos

    enemy_rect = enemy.get_rect() # 적 캐릭터가 갖는 렉탱글 정보를 가져 옴
